// RAG Chatbot API for Portfolio
// Deployed on Vercel as serverless function
// Handles RAG pipeline + Groq API integration
...
function buildRAGPrompt(userQuery, contexts, portfolioData) {
  const name = portfolioData.profile?.name || "Saroj Debnath";
  const title = portfolioData.hero?.title || "Robotic Vision Engineer & LLMOps Specialist";
  ...
  try {
    ...
    if (!GROQ_API_KEY) {
      return res.status(500).json({ 
        error: 'Groq API key not configured. Please set GROQ_API_KEY environment variable in Vercel.' 
      });
    }
    ...
    return res.status(200).json({
      message: aiMessage,
      contexts: contexts.map(ctx => ctx.source) // For debugging
    });
